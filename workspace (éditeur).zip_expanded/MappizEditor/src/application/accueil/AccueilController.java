...
public class AccueilController {
	...
	private void initialize() {
		...
		while (!dossierDesThemes.exists()) {
			String phrase = "Le dossier contenant les thèmes est introuvable. Il a été spécifié comme étant localisé à l'URL \"" + dossierDesThemes.getPath() + "\".\nPour continuer à utiliser l'application, vous devez sélectionner un dossier existant à partir de la fenêtre qui s'ouvrira après avoir cliqué sur \"OK\".";
			Utile.afficherDialogueDInformation("", "Dossier des thèmes introuvable !", phrase, AlertType.ERROR, 550);
			...
			if (!(dossierSelectionne == null)) {
				enregistrerParametres(dossierSelectionne.getPath(), Theme.getNbMaxDeQuestions(), Theme.getNbMaxDeReponses());
			}
			dossierDesThemes = new File(Theme.getUrlDossierDesThemes());
		}

		btnModifierTheme.setDisable(Theme.listerLesThemes().length == 0);
	...
	private void clickLogoUniv() {
		if (Utile.afficherDialogueDeConfirmation("", "Site de Le Mans Université", "Souhaitez-vous consulter le site de Le Mans Université ?", "Oui", "Non")) {
			if (Desktop.isDesktopSupported()) {
				try {
					Desktop.getDesktop().browse(new URI("http://www.univ-lemans.fr/fr/index.html"));
				} catch (IOException | URISyntaxException e) {
					Utile.afficherDialogueDInformation("", "Erreur !", "Une erreur est survenue lors de l'ouverture de votre navigateur Internet.", AlertType.ERROR);
				}
			}
		}
	}
		...
		try {
			...
			dialog.setResultConverter(new Callback<ButtonType, Parametre>() {
				...
				public Parametre call(ButtonType b) {
					if (b == buttonTypeOk) {
						return new Parametre(textFDossierThemes.getText(), spinnerNbMaxDeQuestions.getValue(), spinnerNbMaxDeReponses.getValue());
					}
					return null;
				}
			});
	...
	private void enregistrerParametres(String urlDossierDesThemes, int nbMaxDeQuestionsParTheme, int nbMaxDeReponsesParQuestion) {
		...
		Theme.setNbMaxDeQuestions(nbMaxDeQuestionsParTheme);
	}
