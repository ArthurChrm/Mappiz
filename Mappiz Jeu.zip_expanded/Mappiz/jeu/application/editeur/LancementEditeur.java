...
public class LancementEditeur{

	//@Override
	public void demarrage(Stage primaryStage) {
		try {

			// On crée le fichier des paramètres s'il n'existe pas
			creerFichierDesParametres();

			File paramsFile = new File("params_editeur.csv");
			...
			Theme_Editeur.setUrlDossierDesThemes(paramsActuels.urlDossierDesThemes);
			Theme_Editeur.setNbMaxDeQuestions(paramsActuels.nbMaxDeQuestions);
			Theme_Editeur.setNbMaxDeReponses(paramsActuels.nbMaxDeReponses);
			...
			// Paramètrage du Stage principal de l'application.editeur (un seul pour
			// toute
	...
	private void creerFichierDesParametres() {
		...
		if (!paramsFile.exists()) {
			try {
				...
				System.err.println("Erreur lors de la création ou de l'écriture dans le fichier \"params_editeur.csv\".");
				e.printStackTrace();
			}
		}
	}
