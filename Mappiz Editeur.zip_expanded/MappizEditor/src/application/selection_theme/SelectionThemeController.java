...
public class SelectionThemeController {
	...
	private void clicButtonModifierCeTheme(ActionEvent event) throws IOException {
		...
		if (listVThemes.getSelectionModel().getSelectedIndex() == -1) {
			Utile.afficherDialogueDInformation("", "Sélection incorrecte !", "Veuillez devez sélectionner un thème dans la liste.", AlertType.ERROR);
		} else {
			...
			if (Theme.existe(nomDuTheme)) {
				...
				Utile.afficherDialogueDInformation("", "Thème inexistant", "Le thème \"" + nomDuTheme + "\" n'existe pas. Il a peut-être été supprimé par un autre utilisateur.", AlertType.ERROR);
			...
			if (Utile.afficherDialogueDeConfirmation("", "Supprimer le thème ?", "Souhaitez-vous vraiment supprimer le thème nommé \"" + nomDuTheme + "\" ?", "Oui", "Non")) {
				...
				if (!Theme.supprimerUnTheme(nomDuTheme)) {
					...
					if (dossierDuThemeASupprimer.exists()) {
						...
						if (Utile.afficherDialogueDeConfirmation("", "Échec de la suppression !", phrase, "Oui", "Non")) {
